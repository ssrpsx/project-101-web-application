...
function navbar() {
  return (
    <>
      <div className='shadow-md bg-white text-gray-800 dark:bg-gray-800 dark:text-white z-40'>
        <div className='bg-[#4E71FF] dark:bg-[#5313c4] px-7 py-3'>  
            ...
            <div className='flex justify-between items-center gap-4'>
              <div className='relative group items-center gap3'>
                <input 
                  type="Text"
                  placeholder="Search"
                  className='w-[200px] group-hover:w-[350px] transition-all px-3 py-1.5 pl-4 rounded-full bordor bordor-gray-300 bg-white dark:bg-gray-800 text-black dark:text-white duration-200'
                />
        ...
        <div className='flex justify-center'>
          <ul className='sm:flex hidden items-center gap-4'>
            {Menu.map((data) =>(
              <li key={data.id}>
                <a href={data.link} className='inline-block px-4 py-2 hover:text-[#ff9d1a] font-semibold'>{data.name}</a>
              </li>
            ))}
          </ul>
        </div>
